...
BoxStyle parse_box_style(const json& value)
{
    ...
    if (name == "full")
        return BoxStyle::Full;
    if (name == "corner")
        return BoxStyle::Corner;
...
SnaplineOrigin parse_snapline(const json& value)
{
    const std::string name = value.get<std::string>();
    if (name == "bottom")
        return SnaplineOrigin::Bottom;
    if (name == "center")
        return SnaplineOrigin::Center;
    if (name == "crosshair")
        return SnaplineOrigin::Crosshair;
    throw std::invalid_argument("invalid snapline origin");
}

const char* aim_target_point_name(AimTargetPoint value)
{
    switch (value)
    {
    case AimTargetPoint::Automatic:
        return "automatic";
    case AimTargetPoint::Head:
        return "head";
    case AimTargetPoint::Torso:
        return "torso";
    case AimTargetPoint::Base:
        return "base";
    }
    return "automatic";
}

AimTargetPoint parse_aim_target_point(const json& value)
{
    const std::string name = value.get<std::string>();
    if (name == "automatic")
        return AimTargetPoint::Automatic;
    if (name == "head")
        return AimTargetPoint::Head;
    if (name == "torso")
        return AimTargetPoint::Torso;
    if (name == "base")
        return AimTargetPoint::Base;
    throw std::invalid_argument("invalid aim target point");
}
...
{
    return {{"background", rgba_json(value.background)},
            {"surface", rgba_json(value.surface)},
            {"accent", rgba_json(value.accent)},
            {"text", rgba_json(value.text)},
            {"muted", rgba_json(value.muted)}};
}
...
bool get_clipboard_text(std::string& text, std::string& error)
{
    ...
    const int utf8_size =
        WideCharToMultiByte(CP_UTF8, 0, source, static_cast<int>(length), nullptr, 0, nullptr, nullptr);
...
std::string ConfigManager::serialize(const AppSettings& value)
{
    ...
    json document{
        ...
        {"interface",
         {{"streamproof", value.controls.streamproof},
          {"menu_opacity", value.controls.menu_opacity},
          {"menu_hotkey", value.controls.menu_hotkey},
          {"unload_hotkey", value.controls.unload_hotkey},
          {"aim_toggle_hotkey", value.controls.aim_toggle_hotkey},
          {"esp_toggle_hotkey", value.controls.esp_toggle_hotkey}}},
        {"aim",
         {{"enabled", value.aim.enabled},
          {"always_on", value.aim.always_on},
          {"ignore_fov", value.aim.ignore_fov},
          {"ignore_spawn_protected_targets", value.aim.ignore_spawn_protected_targets},
          {"target_teammates", value.aim.target_teammates},
          {"wallbang", value.aim.wallbang},
          {"fov", value.aim.fov},
          {"hotkey", value.aim.hotkey},
          {"target_point", aim_target_point_name(value.aim.target_point)}}},
        {"esp",
         {{"enabled", esp.enabled},
          {"boxes", esp.boxes},
          {"box_style", box_style_name(esp.box_style)},
          {"filled_boxes", esp.filled_boxes},
          {"fill_opacity", esp.fill_opacity},
          {"box_thickness", esp.box_thickness},
          {"names", esp.names},
          {"show_distance", esp.show_distance},
          {"text_scale", esp.text_scale},
          {"snaplines", esp.snaplines},
          {"snapline_origin", snapline_name(esp.snapline_origin)},
          {"offscreen_arrows", esp.offscreen_arrows},
          {"target_marker", esp.target_marker},
          {"maximum_distance", esp.maximum_distance},
          {"show_team", esp.show_team},
          {"show_enemies", esp.show_enemies},
          {"show_aim_fov", esp.show_aim_fov},
          {"team_color", color_json(esp.team_color)},
          {"enemy_color", color_json(esp.enemy_color)},
          {"target_color", color_json(esp.target_color)},
          {"aim_fov_color", color_json(esp.aim_fov_color)},
          {"aim_fov_thickness", esp.aim_fov_thickness},
          {"aim_fov_opacity", esp.aim_fov_opacity}}},
        {"weapons",
         {{"no_spread", value.weapons.no_spread},
          {"infinite_ammo", value.weapons.infinite_ammo},
          {"instant_reload", value.weapons.instant_reload},
          {"reload_time", value.weapons.reload_time},
          {"no_camera_shake", value.weapons.no_camera_shake},
          {"rapid_fire", value.weapons.rapid_fire}}},
        {"movement",
         {{"auto_sprint", value.movement.auto_sprint},
          {"no_gravity", value.movement.no_gravity},
          {"custom_gravity", value.movement.custom_gravity},
          {"high_speed", value.movement.high_speed},
          {"gravity", value.movement.gravity},
          {"speed", value.movement.speed}}},
        {"player",
         {{"custom_fov", value.player.custom_fov},
          {"disable_spawn_protection", value.player.disable_spawn_protection},
          {"movement_diagnostics", value.player.movement_diagnostics},
          {"camera_fov", value.player.camera_fov}}},
    };
...
ConfigValidation ConfigManager::deserialize(const std::string& document)
{
    ...
    try
    {
        ...
        {
            result.message = version > schema_version ? "Configuration uses an unsupported future version."
                                                      : "Configuration version is unsupported.";
            return result;
        }
        AppSettings temporary;
        temporary.theme = parse_theme(root.at("theme"));
        if (root.contains("custom_theme"))
            temporary.custom_theme = parse_custom_theme(root.at("custom_theme"));
        if (root.contains("interface"))
        {
            const json& interface = root.at("interface");
            temporary.controls.streamproof = interface.value("streamproof", false);
        ...
        temporary.aim.target_teammates = aim.value("target_teammates", true);
        ...
        temporary.aim.target_point = parse_aim_target_point(aim.value("target_point", "automatic"));

        const std::array hotkeys{temporary.controls.menu_hotkey, temporary.controls.unload_hotkey,
                                 temporary.controls.aim_toggle_hotkey, temporary.controls.esp_toggle_hotkey,
                                 temporary.aim.hotkey};
        ...
        {
            if (hotkeys[index] != 0 && !is_bindable_hotkey(hotkeys[index]))
                throw std::invalid_argument("hotkey is reserved");
        }
        for (size_t left = 0; left < hotkeys.size(); ++left)
        {
            ...
            {
                if (hotkeys[left] == hotkeys[right])
                    throw std::invalid_argument("hotkeys must be unique");
            }
        }
        ...
        temporary.player.movement_diagnostics = player.value("movement_diagnostics", false);
        temporary.player.camera_fov = feature_limits::camera_fov(player.at("camera_fov").get<float>());

        const std::array finite_values{temporary.controls.menu_opacity, temporary.aim.fov,
                                       temporary.esp.fill_opacity,      temporary.esp.box_thickness,
                                       temporary.esp.text_scale,        temporary.esp.maximum_distance,
                                       temporary.esp.aim_fov_thickness, temporary.esp.aim_fov_opacity,
                                       temporary.movement.gravity,      temporary.movement.speed,
                                       temporary.player.camera_fov};
        ...
        result.ok = true;
        result.message = std::string("Valid version 1 configuration: theme ") +
                         std::string(theme_key(temporary.theme)) + ", aim " +
                         (temporary.aim.enabled ? "enabled" : "disabled") + ", ESP " +
                         (temporary.esp.enabled ? "enabled." : "disabled.");
...
bool ConfigManager::valid_profile_name(const std::string& name, std::string& error)
{
    ...
    const std::array reserved{"CON",  "PRN",  "AUX",  "NUL",  "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7",
...
bool ConfigManager::write_atomic(const std::filesystem::path& path, const std::string& contents,
...
const std::string& ConfigManager::active_profile() const
{
    return active_profile_;
}
bool ConfigManager::dirty(const AppSettings& live) const
{
    return !(live == saved_);
}
...
{
    if (!valid_profile_name(name, error))
        return false;
    ...
    if (!write_atomic(path_for(name), serialize(live), error))
        return false;
...
{
    if (!write_atomic(path_for(active_profile_), serialize(live), error))
        return false;
    saved_ = live;
    return true;
}
...
{
    AppSettings temporary;
    if (!read_profile(name, temporary, error))
        return false;
    active_profile_ = name;
    live = temporary;
    saved_ = temporary;
...
void ConfigManager::reset(AppSettings& live) const
{
    live = AppSettings{};
}
