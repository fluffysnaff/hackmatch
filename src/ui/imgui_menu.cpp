...
constexpr ui::NavigationIcon page_icons[] = {ui::NavigationIcon::Aim, ui::NavigationIcon::Visuals,
                                             ui::NavigationIcon::Weapons, ui::NavigationIcon::Player,
                                             ui::NavigationIcon::Config};
...
std::string key_name(int key)
{
    ...
    if (extended)
        parameter |= 1L << 24;
    char name[64]{};
    if (scan != 0 && GetKeyNameTextA(parameter, name, static_cast<int>(std::size(name))) > 0)
        return name;
...
{
    if (hotkey_capture.wait_for_release != 0 && (GetAsyncKeyState(hotkey_capture.wait_for_release) & 0x8000) == 0)
    {
...
void hotkey_setting(const char* label, int& key, std::initializer_list<int> conflicts, bool allow_clear = true)
{
    ...
    const std::string button_label =
        (hotkey_capture.id == id ? std::string("Press a key...") : key_name(key)) + "##" + label;
    ...
    for (int candidate = 1; candidate <= 255; ++candidate)
    {
        ...
        if (!is_bindable_hotkey(candidate) || conflict)
            continue;
        key = candidate;
        hotkey_capture.id = 0;
        hotkey_capture.wait_for_release = candidate;
...
void render_card_columns(const char* id, std::initializer_list<int> desktop_columns, Render render)
{
    const int columns = page_column_count();
    if (!ImGui::BeginTable(id, columns, ImGuiTableFlags_SizingStretchSame))
        return;
    if (columns == 1)
    {
        ImGui::TableNextColumn();
        for (int card = 0; card < static_cast<int>(desktop_columns.size()); ++card)
            render(card);
    }
    else
    {
        for (int column = 0; column < columns; ++column)
        {
            ImGui::TableNextColumn();
            int card = 0;
            for (int desktop_column : desktop_columns)
            {
                if (desktop_column == column)
                    render(card);
                ++card;
            }
        }
    ...
    render_card_columns(
        ...
        [&](int card)
        {
            if (card == 0)
            {
                ...
                ui::toggle("Target teammates", "Off when team data is known", aim.target_teammates);
            ...
            else
            {
                ...
                hotkey_setting("Activation key", aim.hotkey,
                               {config.controls.menu_hotkey, config.controls.unload_hotkey,
                ...
                ImGui::EndDisabled();
                int target_point = static_cast<int>(aim.target_point);
                const char* target_points[] = {"Automatic", "Head", "Torso", "Base"};
                ImGui::TextUnformatted("Target point");
                ImGui::SetNextItemWidth(-1.0f);
                if (ImGui::Combo("##target-point", &target_point, target_points, 4))
                {
                    aim.target_point = static_cast<AimTargetPoint>(target_point);
                }
                ui::card_end();
            }
        });
...
void render_visuals_page(EspSettings& esp)
{
    ...
    render_card_columns("visual-columns", {0, 1, 0, 1, 0},
                        [&](int card)
                        {
                            if (card == 0)
                            {
                                ...
                                if (esp.boxes)
                                {
                                    ...
                                    ImGui::SetNextItemWidth(-1.0f);
                                    if (ImGui::Combo("##box-style", &style, styles, 2))
                                        esp.box_style = static_cast<BoxStyle>(style);
                            ...
                            else if (card == 2)
                            {
                                ...
                                ImGui::SetNextItemWidth(-1.0f);
                                if (ImGui::Combo("##snapline-origin", &origin, origins, 3))
                                    esp.snapline_origin = static_cast<SnaplineOrigin>(origin);
                                ImGui::EndDisabled();
                                ui::toggle("Offscreen arrows", "Point toward included players outside the viewport",
                                           esp.offscreen_arrows);
                                ui::toggle("Aim target marker", "Mark the target currently selected by aim assist",
                                           esp.target_marker);
                                ui::card_end();
                            }
                            else if (card == 3)
                            {
                                ...
                                ui::setting_slider("Maximum distance", esp.maximum_distance, 0.0f, 5000.0f,
                                                   esp.maximum_distance <= 0.0f ? "Off" : "%.0f m");
                                ui::card_end();
                            }
                            else
                            {
...
void render_weapons_page(WeaponSettings& weapons)
{
    ...
    render_card_columns("weapon-columns", {0, 1},
                        [&](int card)
                        {
                            if (card == 0)
                            {
                                ui::card_begin("weapons-handling", "Weapon handling",
                                               "Apply reversible changes to equipped item data.");
                                ui::toggle("No spread", "Remove player and projectile spread", weapons.no_spread);
                                ui::toggle("Infinite ammo", "Experimental - restores the pre-enable values",
                                           weapons.infinite_ammo, &caution_background, &ui::active_palette().warning);
                            ...
                            else
                            {
                                ...
                                ui::toggle("Rapid fire", "HIGH BAN RISK - May directly cause bans", weapons.rapid_fire,
    ...
    render_card_columns(
        ...
        [&](int card)
        {
            if (card == 0)
            {
                ...
                ui::toggle("High speed", "Preserve normal collision checks", movement.high_speed);
            ...
            else if (card == 1)
            {
                ...
                ui::toggle("Custom camera FOV", "Override only the local controller camera", player.custom_fov);
                ImGui::BeginDisabled(!player.custom_fov);
                ui::setting_slider("Camera field of view", player.camera_fov, 40.0f, 140.0f, "%.0f deg");
                ImGui::EndDisabled();
                ui::toggle("Disable own spawn protection", "Clear local shield state", player.disable_spawn_protection);
                ui::card_end();
            }
            else
            {
                ui::card_begin("player-diagnostics", "Movement Graph", "Read-only graph rendered in the HUD");
                ui::toggle("HUD graph", nullptr, player.movement_diagnostics);
                ui::card_end();
            }
        });
}
...
bool ImGuiMenu::init(HWND window, ID3D11Device* device, ID3D11DeviceContext* context)
{
    ...
    if (GetWindowsDirectoryA(windows_directory, MAX_PATH) > 0)
    {
        ...
        if (!body_font_)
            body_font_ = io.Fonts->AddFontDefault();
    ...
    if (!body_font_)
        body_font_ = io.Fonts->AddFontDefault();
    if (!title_font_)
        title_font_ = strong_font_ ? strong_font_ : body_font_;
    if (!strong_font_)
        strong_font_ = body_font_;
    io.FontDefault = body_font_;

    BOOL client_animations = TRUE;
    motion_enabled_ =
        !SystemParametersInfoW(SPI_GETCLIENTAREAANIMATION, 0, &client_animations, 0) || client_animations != FALSE;
...
{
    if (page == page_ || page < 0 || page >= static_cast<int>(std::size(page_titles)))
        return;
    previous_page_ = page_;
    for (int index = 0; index < 5; ++index)
        navigation_start_mix_[index] = navigation_mix_[index];
    ...
    content_start_offset_ =
        motion_enabled_ ? (content_was_animating ? content_offset_ : (page > page_ ? 12.0f : -12.0f)) : 0.0f;
...
void ImGuiMenu::perform_profile_action(int action, const std::string& profile)
{
    ...
    bool ok = false;
    if (action == 1)
        ok = config_manager().load(profile, settings(), error);
    else if (action == 2)
        ok = config_manager().remove(profile, settings(), error);
    if (ok)
    {
        ui::apply_theme(settings().theme);
...
void ImGuiMenu::request_profile_action(int action, const std::string& profile)
{
    if (config_manager().dirty(settings()))
    {
        ...
        perform_profile_action(action, profile);
}

void ImGuiMenu::render_config_page(AppSettings& config)
{
    render_card_columns(
        "config-columns", {0, 1, 0, 1},
        [&](int card)
        {
            if (card == 0)
            {
                ...
                ui::toggle("Streamproof", "Hide game window from capture", config.controls.streamproof);
            ...
            else if (card == 1)
            {
                ...
                if (ImGui::BeginCombo("##profile", active.c_str()))
                {
                    ...
                    {
                        if (ImGui::Selectable(profile.c_str(), profile == active) && profile != active)
                            request_profile_action(1, profile);
                    }
                    ImGui::EndCombo();
                }
                ImGui::SameLine();
                const bool dirty = config_manager().dirty(config);
                ImGui::TextColored(dirty ? ui::active_palette().warning : ui::active_palette().success,
                                   dirty ? "Unsaved" : "Saved");
                if (ImGui::Button("New"))
                {
                    profile_name_[0] = '\0';
                ...
                if (ImGui::Button("Delete"))
                    request_profile_action(2, active);
                ImGui::EndDisabled();
                if (ImGui::Button("Reset"))
                {
                    config_manager().reset(config);
                    ui::apply_theme(config.theme);
                    profile_status_ = "Defaults applied; save to keep them.";
                }
                ImGui::SameLine();
                if (ImGui::Button("Copy current config"))
                {
                    std::string error;
                    profile_status_ =
                        config_manager().copy_to_clipboard(config, error) ? "Current configuration copied." : error;
                }
                ImGui::SameLine();
            ...
            else if (card == 2)
            {
                ...
                hotkey_setting("Menu", config.controls.menu_hotkey,
                               {config.controls.unload_hotkey, config.controls.aim_toggle_hotkey,
                                config.controls.esp_toggle_hotkey, config.aim.hotkey},
                               false);
                hotkey_setting("Unload", config.controls.unload_hotkey,
                               {config.controls.menu_hotkey, config.controls.aim_toggle_hotkey,
                                config.controls.esp_toggle_hotkey, config.aim.hotkey},
                               false);
                hotkey_setting("Toggle aim", config.controls.aim_toggle_hotkey,
                               {config.controls.menu_hotkey, config.controls.unload_hotkey,
                                config.controls.esp_toggle_hotkey, config.aim.hotkey});
                hotkey_setting("Toggle ESP", config.controls.esp_toggle_hotkey,
                               {config.controls.menu_hotkey, config.controls.unload_hotkey,
                                config.controls.aim_toggle_hotkey, config.aim.hotkey});
                ui::card_end();
            }
            else
            {
                ...
                ImGui::PopFont();
                ImGui::Text("Redmatch 2 build %.*s", static_cast<int>(game_offsets::supported_build.size()),
                            game_offsets::supported_build.data());
                ...
                {
                    ShellExecuteA(nullptr, "open", "https://github.com/fluffysnaff/hackmatch", nullptr, nullptr,
                                  SW_SHOWNORMAL);
                }
                ImGui::PopStyleColor();
                ImGui::EndGroup();
    ...
    if (ImGui::BeginPopupModal("New profile", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            std::string error;
            if (config_manager().create(profile_name_, config, error))
            {
                profile_status_ = "Profile created.";
    ...
    if (ImGui::BeginPopupModal("Rename profile", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            std::string error;
            if (config_manager().rename_active(profile_name_, error))
            {
                profile_status_ = "Profile renamed.";
                ImGui::CloseCurrentPopup();
            }
            else
                profile_status_ = error;
...
void ImGuiMenu::render_options()
{
    ...
    if (!ImGui::Begin("Hackmatch##main", &visible_,
    ...
    root_draw->AddRectFilled(window_position, {window_position.x + sidebar_width, window_max.y}, packed(color.sidebar),
    ...
    ImGui::PushFont(strong_font_);
    ImGui::TextUnformatted("Hackmatch");
    ImGui::PopFont();
    ...
    draw->AddRectFilled({sidebar_position.x + 14.0f, indicator_y + 8.0f},
                        {sidebar_position.x + 17.0f, indicator_y + 40.0f}, packed(color.accent), 2.0f);
    for (int index = 0; index < 5; ++index)
    {
        ImGui::SetCursorPos({navigation_x, navigation_y + navigation_step * static_cast<float>(index)});
        if (ui::navigation_item(page_titles[index], page_icons[index], navigation_mix_[index]))
            start_page_transition(index);
    }
...
#endif
    const ImVec2 status_min{sidebar_position.x + 20.0f,
                            sidebar_position.y + window_size.y - (build_warning ? 124.0f : 116.0f)};
    ...
    ImGui::PushFont(title_font_);
    ImGui::TextUnformatted(page_titles[page_]);
    ImGui::PopFont();
    ...
    draw->AddLine({content_position.x + content_x, content_position.y + 92.0f + content_y},
                  {content_position.x + window_size.x - sidebar_width - 32.0f, content_position.y + 92.0f + content_y},
                  packed(color.border));
    ImGui::SetCursorPos({content_x, 112.0f + content_y});
    ImGui::BeginChild("page-scroll",
                      {window_size.x - sidebar_width - content_x - 24.0f, window_size.y - 132.0f - content_y},
                      ImGuiChildFlags_None);
    switch (page_)
    {
    case 0:
        render_aim_page(config);
        break;
    case 1:
        render_visuals_page(config.esp);
        break;
    case 2:
        render_weapons_page(config.weapons);
        break;
    case 3:
        render_player_page(config.movement, config.player);
        break;
    case 4:
    ...
    ImGui::EndChild();
    ImGui::PopStyleColor();
    ImGui::GetForegroundDrawList()->AddRect({window_position.x + 0.5f, window_position.y + 0.5f},
                                            {window_max.x - 0.5f, window_max.y - 0.5f}, packed(color.border_strong),
                                            9.5f, 0, 1.0f);
    ImGui::End();
    ImGui::PopStyleVar();
}
...
void ImGuiMenu::update_cursor()
{
    ...
    if (!visible_ || !focused)
    {
        if (cursor_mode_ != CursorMode::Inactive)
        {
            cursor_mode_ = CursorMode::Inactive;
            virtual_mouse_ready_ = false;
        }
    ...
    if (requested_mode != cursor_mode_)
    {
        cursor_mode_ = requested_mode;
        virtual_mouse_ready_ = false;
    }
    if (cursor_mode_ == CursorMode::Native)
    {
        ...
        {
            last_mouse_x_ = static_cast<float>(position.x);
            last_mouse_y_ = static_cast<float>(position.y);
            last_mouse_valid_ = true;
            io.AddMousePosEvent(last_mouse_x_, last_mouse_y_);
        }
        else
            io.AddMousePosEvent(-FLT_MAX, -FLT_MAX);
        io.MouseDrawCursor = false;
    ...
    last_mouse_x_ = virtual_mouse_x_;
    last_mouse_y_ = virtual_mouse_y_;
    last_mouse_valid_ = true;
    io.MouseDrawCursor = true;
    io.AddMousePosEvent(virtual_mouse_x_, virtual_mouse_y_);
}
...
bool ImGuiMenu::visible() const
{
    return visible_;
}
bool ImGuiMenu::capturing_hotkey() const
{
    return hotkey_capture.suppress_actions;
}
void ImGuiMenu::add_mouse_delta(float x, float y)
{
    if (visible_ && cursor_mode_ == CursorMode::Virtual && virtual_mouse_ready_)
    {
        virtual_mouse_x_ += x;
        virtual_mouse_y_ += y;
    }
...
void ImGuiMenu::set_preview_state(int theme, int page, float navigation_progress, float content_progress,
                                  bool reduced_motion)
{
