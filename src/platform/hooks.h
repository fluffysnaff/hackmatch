...
class Hooks
{
  ...
  private:
    ...
    {
      public:
        explicit CallbackGuard(Hooks& owner);
        ~CallbackGuard();
    ...
    static HRESULT __stdcall resize_buffers_hook(IDXGISwapChain* swap_chain, UINT buffer_count, UINT width, UINT height,
                                                 DXGI_FORMAT format, UINT flags);
    static void fire_primary_shot_hook(il2cpp::Object* player, il2cpp::Object* shot, const MethodInfo* method);
    static il2cpp::Array* raycast_all_hook(Vector3 origin, Vector3 direction, float max_distance, int layer_mask,
    ...
    void release_device();
    void update_display_affinity();
    bool setup_menu(IDXGISwapChain* swap_chain);
    HRESULT present(IDXGISwapChain* swap_chain, UINT sync_interval, UINT flags);
    HRESULT resize_buffers(IDXGISwapChain* swap_chain, UINT buffer_count, UINT width, UINT height, DXGI_FORMAT format,
                           UINT flags);
    void shot(ShotFn original, il2cpp::Object* player, il2cpp::Object* shot, const MethodInfo* method);
    bool clear_shot(Vector3 origin, Vector3 direction, float max_distance, int layer_mask,
                    int query_trigger_interaction, const MethodInfo* method);
    il2cpp::Array* raycast_all(Vector3 origin, Vector3 direction, float max_distance, int layer_mask,
                               int query_trigger_interaction, const MethodInfo* method);
    ...
    ShotFn fire_primary_shot_ = nullptr;
    ...
    std::atomic<bool> unload_{false};
    bool menu_ready_ = false;
    bool menu_failure_logged_ = false;
    bool display_affinity_initialized_ = false;
    bool display_affinity_failure_logged_ = false;
    bool streamproof_applied_ = false;
    bool minhook_ready_ = false;
    std::atomic<bool> shutting_down_{false};
    int active_callbacks_ = 0;
    std::mutex callback_mutex_;
    std::condition_variable callback_idle_;
    std::mutex player_update_mutex_;
