...
{
    if (get_transform_ && get_position_ && get_main_camera_ && get_current_camera_ && world_to_viewport_ && object_equals_) {
        return true;
    }
    ...
    world_to_viewport_ = il2cpp::method("UnityEngine.CoreModule", "UnityEngine", "Camera", "WorldToViewportPoint", 1);
    get_shared_material_ = il2cpp::method("UnityEngine.CoreModule", "UnityEngine", "Renderer", "get_sharedMaterial", 0);
    ...
    return get_transform_ && get_position_ && get_main_camera_ && get_current_camera_ && world_to_viewport_ && object_equals_;
}
...
std::string EspPlayers::name_from_identity(il2cpp::Object* identity)
{
    ...
    return utf8(*reinterpret_cast<Il2CppString**>(reinterpret_cast<char*>(player_data) + game_offsets::fields::player_data_name));
}

il2cpp::Object* EspPlayers::player_data_from_identity(il2cpp::Object* identity)
{
    return identity ? *reinterpret_cast<il2cpp::Object**>(reinterpret_cast<char*>(identity) + game_offsets::fields::identity_player_data) : nullptr;
}
...
void EspPlayers::scan_team_info()
{
    ...
    for (int i = 0; i < total && team_count_ < max_team_entries; ++i) {
        ...
        for (int j = 0; j < team_total && team_count_ < max_team_entries; ++j) {
            ...
            TeamEntry& entry = team_entries_[team_count_++];
            entry = {};
            il2cpp::read_field(team, team_id_, &entry.team);
            il2cpp::read_field(team, team_outline_material_, &entry.material);
        }
    }
}
...
void EspPlayers::scan_outlines(il2cpp::Object* local)
{
    ...
    for (int i = 0; i < total && outline_count_ < max_outline_entries; ++i) {
        ...
        OutlineEntry& entry = outline_entries_[outline_count_++];
        entry = {};
        il2cpp::Object* renderer = nullptr;
        il2cpp::read_field(outline, outline_player_, &entry.player);
...
bool EspPlayers::world_to_screen(const Vec3& world, ImVec2& out, bool& in_front)
{
    ...
    if (!alive(camera) || exception) {
        return false;
    }

    void* args[] = {const_cast<Vec3*>(&world)};
    exception = nullptr;
    il2cpp::Object* boxed = il2cpp::runtime_invoke(world_to_viewport_, camera, args, &exception);
    if (!boxed || exception) {
        return false;
    }

    Vec3 viewport = unbox<Vec3>(boxed);
    const ImVec2 display = ImGui::GetIO().DisplaySize;
    out = {viewport.x * display.x, (1.0f - viewport.y) * display.y};
    in_front = viewport.z > 0.01f;
    if (!in_front) {
...
void EspPlayers::update_players()
{
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        const bool head_projected = world_to_screen(head, player.head, head_front);
        const bool feet_projected = world_to_screen(feet, player.feet, feet_front);
        const bool center_projected = world_to_screen(player.center, player.screen_center, center_front);
        player.projected = head_projected && feet_projected && center_projected;
        const ImVec2 display = ImGui::GetIO().DisplaySize;
        player.visible = player.projected && head_front && feet_front && center_front && player.feet.y > player.head.y &&
            player.head.x >= 0.0f && player.head.x <= display.x && player.head.y >= 0.0f && player.feet.y <= display.y;
    }
}
